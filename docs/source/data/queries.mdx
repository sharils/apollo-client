...
of this information, we set the `notifyOnNetworkStatusChange` option to `true` so our [query component](https://www.apollographql.com/docs/react/api/react/components/ "Components - Client (React) - Apollo GraphQL Docs") re-renders while a refetch is in flight:
